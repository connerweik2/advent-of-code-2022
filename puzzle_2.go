...
func main() {
	...
	productOfDivisors := int64(1)
	for _, monkey := range monkeys {
		productOfDivisors *= monkey.divisor
	}

	for thisRound := 0; thisRound < 10000; thisRound++ {
		for _, monkey := range monkeys {
			for _, item := range monkey.items {
				...
				if newWorryLevel%monkey.divisor == int64(0) {
					monkeys[monkey.throwToIfTrue].items =
						append(monkeys[monkey.throwToIfTrue].items, newWorryLevel%productOfDivisors)
				} else {
					monkeys[monkey.throwToIfFalse].items =
						append(monkeys[monkey.throwToIfFalse].items, newWorryLevel%productOfDivisors)
				}
			}
