...
int main(void) {
        ...
        for (auto monkey : monkeys) {
            for (auto item : monkey->items) {
                ...
                if (new_worry_level % monkey->divisor == 0ll) {
                    monkeys[monkey->throw_to_if_true]->items.push_back(new_worry_level);
                } else {
                    monkeys[monkey->throw_to_if_false]->items.push_back(new_worry_level);
                }
            }
    ...
    cout << (long)monkey_activity[monkey_activity.size() - 2] * (long)monkey_activity[monkey_activity.size() - 1] << endl;
}